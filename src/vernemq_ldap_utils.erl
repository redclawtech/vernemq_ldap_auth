...
open_connection([Host|OtherHosts], Port, SSL, Timeout, Opts) ->
    SSLOpts = case SSL of
		  true -> [{ssl, true}, {sslopts, ssl_options(Host, Opts)}];
		  false -> []
	      end,
	...
	{error, _} -> open_connection(OtherHosts, Port, SSL, Timeout, Opts)
    end.
...
eldap_search(Handle, SearchProps) ->
	...
	{ok, #eldap_search_result{
		entries = Entries,
		referrals = Refs}} ->
	    Refs == [] orelse lager:error("LDAP search continuations are not "
